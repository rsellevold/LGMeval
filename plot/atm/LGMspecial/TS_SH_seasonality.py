...
# Load data
fname = f"{config['run']['folder']}/{config['run']['name']}/atm/hist/DJFavg/ICEFRAC.{config['run']['ystart']}-{config['run']['yend']}.nc"
...
obs_TS = [float(i.replace("−","-")) for i in list(df["E-LGM SSST (°C)"][:])]
print(obs_TS)
sys.exit()
